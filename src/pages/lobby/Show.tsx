...
export default function Show() {
  const { id } = useParams();
  const { lobby, loading, startGame } = useLobby(id);
  const { isLoading: authLoading } = useProtectedRoute();

  const isOwner = useMemo(() => {
    if (!lobby?.current_player.id) return false;
    return lobby?.owner.id === lobby?.current_player.id;
  }, [lobby]);

  const currentParticipant = useMemo(() =>
    lobby?.participants.find(p => p.id === lobby?.current_player.id),
    [lobby]
  );
