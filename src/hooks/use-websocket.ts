...
export const useGameWebSocket = ({
	...
	useEffect(() => {
		const wsUrl = `${window.location.protocol === 'https:' ? 'ws:' : 'ws:'}//api.troika.id.lv/games/${gameId}`;
		// const wsUrl = `${window.location.protocol === 'https:' ? 'ws:' : 'ws:'}//localhost:8000/games/${gameId}`;
